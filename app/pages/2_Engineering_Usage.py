...
from telemetry_analytics.metrics.queries import ENGINEERING_COHORT_SQL


st.set_page_config(page_title="Engineering Usage", page_icon=":bar_chart:", layout="wide")
st.title("Engineering Usage")
...
with connect(settings.db_path, read_only=True) as conn:
    cohorts = conn.execute(ENGINEERING_COHORT_SQL).fetchdf()
